...
最初の対象は **社会保障審議会 医療保険部会 第215回（2026年9月11日）** の3資料
...
最後まで残っていた「**医療法に関する三党合意書**」（2025年6月6日）は、
...
`make validate` は ERROR 0 で通るが、WARN が171件残っている。内訳は3種類である。

| 件数 | 内容 | 扱い |
|---|---|---|
| 105 | 原典の抽出テキストにない数値 | 多くは本解説での計算・他スライドからの引用。**数値照合（v1.4）で潰す** |
...
- **年月は西暦で書く**（SPEC §6 品質ルール8）。和暦が残るのは原典の文言をそのまま引く箇所だけで、
  スライドや文書のタイトル・法律番号・引用ブロック・「」内・【原典ママ】の行がそれにあたる。
  本文に和暦が残っていれば `validate.py` が警告する。
